...
contract OrderbookOracle is Ownable {
    ...
    function getData(
        ...
        int24 coeffVariantTick = int24(
            int256((uint256(coeffVariants[index / 10]) >> (256 - (24 * (internalIndex + 1)))))
        );
    ...
    function buildUpdateData(int24[] calldata _depths) external pure returns (bytes32[] memory _updateData) {
        ...
        for (uint256 i; i < _depths.length; ++i) {
            ...
            bytes32 partialWord = bytes32(
                uint256(uint24(_depths[i])) << (24 * (MAX_DEPTH_PER_WORD - 1 - innerIndex) + 16)
            );
