...
contract PerpStorage is OwnableUpgradeable, ReentrancyGuardUpgradeable, IPerpStorage {
    ...
    function getPositionBySubAccount(
        address _subAccount
    ) external view returns (Position[] memory _subAccountPositions) {
    ...
    function getActiveSubAccounts(
        uint256 _limit,
        uint256 _offset
    ) external view returns (address[] memory _subAccounts) {
    ...
    function _getCurrentEpochVolumeTimestamp() internal view returns (uint256 epochTimestamp) {
        return
            movingWindowInterval > 0
                ? (block.timestamp / movingWindowInterval) * movingWindowInterval
                : block.timestamp;
    }
