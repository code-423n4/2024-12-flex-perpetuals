...
contract BotHandler is ReentrancyGuardUpgradeable, OwnableUpgradeable, IBotHandler {
    ...
    function checkForceTakeMaxProfit(
        ...
        if (
            _injectedAssetIds.length != _injectedPrices.length ||
            _injectedAssetIds.length == 0 ||
            _injectedPrices.length == 0
    ...
    function checkLiquidation(
        ...
        int256 _equityValueE30 = calculator.getEquityWithInjectedPrices(
            _subAccount,
            _injectedAssetIds,
            _injectedPrices
        );
        ...
        for (uint256 _i; _i < _len; ) {
            ...
            if (_stableToken != vars.collatToken) {
                ...
                if (vars.fundingFeeReserve > 0) {
                    (vars.collatTokenPrice, ) = _oracle.getLatestPrice(
                        _configStorage.tokenAssetIds(vars.collatToken),
                        false
                    );
