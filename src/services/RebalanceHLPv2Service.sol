    ...
    IRebalanceHLPv2Service
{
    ...
    function afterDepositCancellation(
        ...
        if (_depositParam.longTokenAmount > 0) {
            vaultStorage.clearOnHold(_depositParam.longToken, _depositParam.longTokenAmount);
            IERC20Upgradeable(_depositParam.longToken).safeTransfer(
                address(vaultStorage),
                _depositParam.longTokenAmount
            );
        ...
        if (_depositParam.shortTokenAmount > 0) {
            ...
            IERC20Upgradeable(_depositParam.longToken).safeTransfer(
                address(vaultStorage),
                _depositParam.shortTokenAmount
            );
    ...
    function afterWithdrawalExecution(
        ...
        if (_receivedLong > 0) {
            IERC20Upgradeable(_eventData.addressItems.items[0].value).safeTransfer(
                address(vaultStorage),
                _receivedLong
            );
        ...
        if (_receivedShort > 0) {
            IERC20Upgradeable(_eventData.addressItems.items[1].value).safeTransfer(
                address(vaultStorage),
                _receivedShort
            );
