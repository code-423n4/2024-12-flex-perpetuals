...
contract OracleMiddleware_GetAdaptivePriceTest is OracleMiddleware_BaseTest {
    ...
    function testCorrectness_WhenGetLatestPrice() external {
        ...
        (uint256 maxPrice, uint256 lastUpdate) = oracleMiddleware.getLatestAdaptivePrice(wbtcAssetId, true, 0, 0, 0, 0);
        (uint256 minPrice, ) = oracleMiddleware.getLatestAdaptivePrice(wbtcAssetId, false, 0, 0, 0, 0);
        ...
        vm.expectRevert();
        oracleMiddleware.getLatestAdaptivePrice("168", true, 0, 0, 0, 0);
    }
    ...
    function testCorrectness_getLatestPrice_premiumPrice() external {
        ...
        // note: unsupported min, max price logic then min & max price should be same
    ...
    function testCorrectness_getLatestPrice_discountPrice() external {
        ...
        // premium (after) = (-100000 + -(7200)) / 1000000 = -0.1072
