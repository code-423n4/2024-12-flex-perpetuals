...
contract Smoke_Collateral is ForkEnv {
    ...
    function _withdrawCollateral() internal {
        ...
        for (uint8 i = 0; i < collateralToken.length; i++) {
            ...
            vm.prank(ALICE);
            uint256 _latestOrderIndex = ForkEnv.crossMarginHandler.createWithdrawCollateralOrder{
                value: minExecutionFee
            }(SUB_ACCOUNT_NO, address(collateralToken[i]), 10 * (10 ** tokenDecimal), minExecutionFee, false);
