...
contract SwitchCollateralRouter_ForkTest is ForkEnv, Cheats {
    ...
    function setUp() external {
        ...
        _uniswapDexter = UniswapDexter(
            address(
                Deployer.deployUniswapDexter(address(ForkEnv.uniswapPermit2), address(ForkEnv.uniswapUniversalRouter))
            )
        );
        ...
        aerodromeDexter = AerodromeDexter(address(Deployer.deployAerodromeDexter(ForkEnv.aerodromeRouter)));
        IRouter.Route[] memory route = new IRouter.Route[](2);
        route[0] = IRouter.Route(address(ForkEnv.usdt), address(ForkEnv.usdc), true, ForkEnv.aerodromePoolFactory);
        route[1] = IRouter.Route(address(ForkEnv.usdc), address(ForkEnv.dai), true, ForkEnv.aerodromePoolFactory);
        aerodromeDexter.setRouteOf(address(ForkEnv.usdt), address(ForkEnv.dai), route);
        ...
        ForkEnv.configStorage.setServiceExecutor(
            address(ForkEnv.crossMarginService),
            address(crossMarginHandler02),
            true
        );
    ...
    function testCorrectness_WhenSwitchCollateralFromWstEthTosglp() external {
        ...
        uint256 _orderIndex = ext01Handler.createExtOrder{value: 0.1 * 1e9}(
            IExt01Handler.CreateExtOrderParams({
                ...
                data: abi.encode(
                    SUB_ACCOUNT_ID,
                    ForkEnv.vaultStorage.traderBalances(USER, address(ForkEnv.wstEth)),
                    _path,
                    0
                )
            })
